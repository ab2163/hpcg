# 1 "CG_stdexec_stripped.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 453 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "CG_stdexec_stripped.cpp" 2
...
int CG_stdexec(const SparseMatrix &A, CGData &data, const Vector &b, Vector &x,
  ...
  sender auto pre_loop_work =
      ...
      bulk(stdexec::par_unseq, nrow,
           [&](local_int_t i) {
             (pVals)[i] = (1) * (xVals)[i] + (0) * (xVals)[i];
           }) |
      then([&]() { t_tmp = mytimer(); }) |
      bulk(stdexec::par_unseq, false ? 0 : (A).localNumberOfRows,
           [&](local_int_t i) {
             if (false) {
               return;
             }
      ...
      bulk(stdexec::par_unseq, nrow,
           [&](local_int_t i) {
             (rVals)[i] = (1) * (bVals)[i] + (-1) * (ApVals)[i];
           }) |
      then([&]() {
        (normr) = std::transform_reduce(std::execution::par, (rVals),
                                        (rVals) + nrow, (rVals), 0.0);
      }) |
      then([&]() {
        normr = sqrt(normr);
  ...
  stdexec::sender auto sndr_first =
      stdexec::schedule(scheduler) | then([&]() { t_tmp = mytimer(); }) |
      bulk(stdexec::par_unseq,
           prolong_flags[indPC] ? 0 : (*Aptrs[indPC]).mgData->rc->localLength,
           [&](int i) {
             if (prolong_flags[indPC]) {
               return;
             }
             if (i >= (A).mgData->rc->localLength) {
               return;
             }
             xfv_ptrs[(indPC)][f2c_ptrs[(indPC)][i]] += xcv_ptrs[(indPC)][i];
           }) |
      then([&]() { t_prolong += mytimer() - t_tmp; }) | then([&]() {
        if (zerovector_flags[indPC])
          ZeroVector(*zptrs[indPC]);
      }) |
      continues_on(scheduler_single_thread) | then([&]() {
        ComputeSYMGS_ref(*Aptrs[indPC], *rptrs[indPC], *zptrs[indPC]);
      }) |
      continues_on(scheduler) | then([&]() { t_tmp = mytimer(); }) |
      bulk(stdexec::par_unseq,
           restrict_flags[indPC] ? 0 : (*Aptrs[indPC]).localNumberOfRows,
           [&](local_int_t i) {
             if (restrict_flags[indPC]) {
               return;
             }
             ...
             double sum = 0.0;
             double *cur_vals = (*Aptrs[indPC]).matrixValues[i];
             local_int_t *cur_inds = (*Aptrs[indPC]).mtxIndL[i];
             int cur_nnz = (*Aptrs[indPC]).nonzerosInRow[i];
             double *xv = (*zptrs[indPC]).values;
             for (int j = 0; j < cur_nnz; j++)
               sum += cur_vals[j] * xv[cur_inds[j]];
             (*((*Aptrs[indPC]).mgData->Axf)).values[i] = sum;
           }) |
      then([&]() { t_SPMV += mytimer() - t_tmp; }) |
      then([&]() { t_tmp = mytimer(); }) |
      bulk(stdexec::par_unseq,
           restrict_flags[indPC] ? 0 : (*Aptrs[indPC]).mgData->rc->localLength,
           [&](int i) {
             if (restrict_flags[indPC]) {
               return;
             }
             if (i >= (*Aptrs[indPC]).mgData->rc->localLength) {
               return;
             }
             rcv_ptrs[(indPC)][i] = rfv_ptrs[(indPC)][f2c_ptrs[(indPC)][i]] -
                                    Axfv_ptrs[(indPC)][f2c_ptrs[(indPC)][i]];
           }) |
      then([&]() { t_restrict += mytimer() - t_tmp; }) |
      then([&]() { indPC++; }) | exec::repeat_n(7) |
      bulk(stdexec::par_unseq, nrow,
           [&](local_int_t i) {
             (pVals)[i] = (1) * (zVals)[i] + (0) * (zVals)[i];
           }) |
      then([&]() {
        (rtz) = std::transform_reduce(std::execution::par, (rVals),
                                      (rVals) + nrow, (zVals), 0.0);
      });

  stdexec::sender auto sndr_second =
      stdexec::schedule(scheduler) | then([&]() { t_tmp = mytimer(); }) |
      bulk(stdexec::par_unseq,
           prolong_flags[indPC] ? 0 : (*Aptrs[indPC]).mgData->rc->localLength,
           [&](int i) {
  ...
  sender auto loop_work =
      let_value(stdexec::schedule(scheduler),
                [&]() {
                  if (k == 2) {
      ...
      then([&]() { t_SPMV += mytimer() - t_tmp; }) | then([&]() {
        (pAp) = std::transform_reduce(std::execution::par, (pVals),
                                      (pVals) + nrow, (ApVals), 0.0);
      }) |
      then([&]() { alpha = rtz / pAp; }) |
      bulk(stdexec::par_unseq, nrow,
           [&](local_int_t i) {
             (xVals)[i] = (1) * (xVals)[i] + (alpha) * (pVals)[i];
           }) |
      bulk(stdexec::par_unseq, nrow,
           [&](local_int_t i) {
             (rVals)[i] = (1) * (rVals)[i] + (-alpha) * (ApVals)[i];
           }) |
      then([&]() {
        (normr) = std::transform_reduce(std::execution::par, (rVals),
                                        (rVals) + nrow, (rVals), 0.0);
