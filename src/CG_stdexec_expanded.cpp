...
int CG_stdexec(const SparseMatrix &A, CGData &data, const Vector &b, Vector &x,
  ...
  sender auto pre_loop_work =
      ...
      bulk(stdexec::seq, false ? 0 : (A).localNumberOfRows, spmv_kernel) |
  ...
  stdexec::sender auto sndr_first =
      stdexec::schedule(sched_inl) | then([&]() { t_tmp = mytimer(); }) |
      ...
      continues_on(scheduler) | then([&]() {
        t_tmp = mytimer();
        A_spmv = &(*Aptrs[indPC]);
        x_spmv = &(*zptrs[indPC]);
        if (!restrict_flags[indPC]) {
          y_spmv = &(*((*Aptrs[indPC]).mgData->Axf));
        }
        disable_spmv = (restrict_flags[indPC]);
      }) |
      bulk(stdexec::seq,
  ...
  sender auto loop_work =
      let_value(stdexec::schedule(sched_inl),
                [&]() {
                  if (k == 2) {
